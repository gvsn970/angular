...
@Component({
  selector: 'app-vehicle-outward',
  templateUrl: './vehicle-outward.component.html',
  styleUrls: ['./vehicle-outward.component.css']
})
export class VehicleOutwardComponent implements OnInit {
  rowData: any;
  modalRef: BsModalRef;
  ...
  getAllTransferRoutesOnLoad() {
    ...
    if (this.userData.locationName) {
      ...
      // currentDate='06-03-2020';
      this.securityService.getAllTransferRoutesOutPage(this.p, this.pageSize, this.userData.locationName, currentDate).subscribe(res => {
  ...
  openModal(template: any, index, openFor) {
    ...
    if (openFor == 'route') {
      this.route = this.routeDetail.transDeliverySetOut[index]
    }
    if (openFor == 'vehicle') {
      this.getVehicleDetails(this.routeDetail.transDeliverySetOut[index].vehicleId);
      this.getDriverDetails(this.routeDetail.transDeliverySetOut[index].driverId);
      this.getVechicleInsurance(this.routeDetail.transDeliverySetOut[index].vehicleId);
      this.getVechicleFitness(this.routeDetail.transDeliverySetOut[index].vehicleId);
    }
    if (openFor == 'seal') {
      this.getSealDetail(this.routeDetail.transDeliverySetOut[index].tripTransactionId);
    }
    if (openFor == 'tripe') {
      this.tripeList = this.routeDetail.transDeliverySetOut[index].tripPoints.sort((a, b) => a.routePointSequenceNo - b.routePointSequenceNo);
      // //console.log('this.tripeList',this.tripeList)
    }
    if (openFor == 'shipment') {
      // this.getSalesOrderShipmentDatail(this.routeDetail.transDeliverySetOut[index].tripTransactionId);
      // this.getMilkAndMaterialShipmentDatail(this.routeDetail.transDeliverySetOut[index].tripTransactionId);
      this.getShippingListByReportDateAndShipStatusAndRoute(this.routeDetail.transDeliverySetOut[index].routeNumber);
    }
  }
  ...
  search() {
    ...
    this.securityService.findAllOutgoingTransferRoutesByFilter(currentDate,
      ...
      this.routeNumber,
      //this.userData.locationId
      this.searchForm.value.dispatchFrom,
    ).subscribe(res => {
      this.searchResult = res;
      if (this.searchResult.length > 0) {
        this.routeDetail = res;
        this.routeDetail.transDeliverySetOut = res;
        this.routeDetail.totalRecords = this.routeDetail.transDeliverySetOut.length;
  ...
  changeStatus(status, tripTransactionId) {
    ...
    this.securityService.updateOutwardTripStatus(postData).subscribe(res => {
