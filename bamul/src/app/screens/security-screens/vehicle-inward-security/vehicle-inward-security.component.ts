...
@Component({
  selector: 'app-vehicle-inward-security',
  templateUrl: './vehicle-inward-security.component.html',
  styleUrls: ['./vehicle-inward-security.component.css']
})
export class VehicleInwardSecurityComponent implements OnInit {
  rowData: any;
  modalRef: BsModalRef;
  routeDetail: any = [];
  userData: any;
  ...
  getAllTransferRoutesOnLoad(){
    ...
    if(this.userData.locationName){
      ...
      this.securityService.getAllTransferRoutesIn(this.userData.locationName,currentDate).subscribe(res => {
