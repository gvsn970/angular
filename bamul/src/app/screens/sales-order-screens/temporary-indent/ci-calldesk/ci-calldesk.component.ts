...
@Component({
  selector: 'app-ci-calldesk',
  templateUrl: './ci-calldesk.component.html',
  styleUrls: ['./ci-calldesk.component.css'],
  providers: [TabService, DecimalPipe]
})
export class CiCalldeskComponent implements OnInit {
  modalRef: BsModalRef;
  cancelled = false;
  panelLeft: any;
  panelRight: any;
  ...
  async getCustomerDetails(elem) {
    ...
    this.callDeskService.getCustomerShipping(elem.target.value).subscribe(async (response) => {
      ...
      if (this.shippingDetails) {
        ...
        this.pricelistId = this.shippingDetails[0].priceListId;
        ...
        if (shipVal !== undefined) {
          await this.callDeskService.getcurrentshiftbytime().subscribe(async (res) => {
            ...
            if(this.shift.shiftCode=='E'){
              this.shift = 'Evening';
              this.getIndentListBySiteUseId(shipVal.siteUseId + '/' + shipVal.siteWarehouseName + '/' + shipVal.salesRepName + '/' + shipVal.siteWareHouseId+ '/' +this.pricelistId)
            } else {
              this.shift = 'Morning';
              this.getIndentListBySiteUseId(shipVal.siteUseId + '/' + shipVal.siteWarehouseName + '/' + shipVal.salesRepName + '/' + shipVal.siteWareHouseId+ '/' +this.pricelistId)
            }
  ...
  async getIndentListBySiteUseId(value) {
    ...
    this.pricelistId = shippingVal[4];
    ...
    let shift=1;
    // console.log('this.callDeskService.getDateAndShift',this.callDeskService.getDateAndShift()['shift'])
    // if(this.callDeskService.getDateAndShift()['shift']=='E'){
    //   shift=2;
    //   this.shift = 'Evening';
    // } else {
    //   this.shift = 'Morning';
    //   shift=1;
    // }
    await this.callDeskService.getSalesOrderDetail(this.siteUseId, this.dateToSend, custId, this.shiftId).subscribe((response) => {
      if (response['length']==0 || response ===null) {
        ...
        this.mapOfOrderByCustomer = new Map(); //for refreshing the list 
        this.mapOfOrderByCustomerUpdate = new Map(); //for refreshing the list 
        this.updateDisabled = false;
        this.mapOfOrderByCustomer = new Map(); //for refreshing the list 
        this.mapOfOrderByCustomerUpdate = new Map(); //for refreshing the list 
        
        for (const order of this.customerDetails.salesOrderLine) {
          // this.bookingDate=this.customerDetails.bookingDate;
          // this.deliveryDate=this.customerDetails.deliveryDate;
        ...
        for (const panelLeft of this.panelLeft) {
          if (this.mapOfOrderByCustomer.has(panelLeft.itemCode)) {
            ...
            panelLeft.qty = this.mapOfOrderByCustomer.get(panelLeft.itemCode)[1];
            //panelLeft.uomOrderQuantityToSend = this.mapOfOrderByCustomer.get(panelLeft.itemCode)[2];
             // changes made as per discussion with ganesh on 08-10-2020
  ...
  descCalc(panelVal) {
    ...
    if (this.shippingDetails != undefined) {
      ...
      if (panelVal.isExist) {
        this.callDeskService.getItemsByPriceListIdAndOrderTypeIdAnditemCodeAndSellingUOM(this.pricelistId, this.customerDetails.orderTypeId, panelVal.itemCode, sellingUOM).subscribe((response) => {
  ...
  updateTotalValue() {
    ...
    for (const panelVal of this.panelRight) {
      ...
      let unitsPerCratesAdd = isNaN(panelVal.unitsPerCrates) ? 0 : panelVal.unitsPerCrates;
      let unitsToAdd = isNaN(panelVal.qty) ? 0 : panelVal.qty;
      const itemPriceAdd = isNaN(panelVal.unitSellingPrice) ? 0 : panelVal.unitSellingPrice;
      panelVal.priceItem = ((cratesToAdd * unitsPerCratesAdd) + unitsToAdd) * itemPriceAdd;
  ...
  async onChangeOfQuantity(panelVal) {
    ...
    this.x = this.x++;
    let oldIsExist = panelVal.isExist;
        ...
        if (res) {
          ...
          if (firstRes !== undefined) {
            ...
            if ((availToOrder <= 0) || (availToOrder === null)) {
              ...
              if (availToOrder != null || availToOrder != undefined) {
                //  if (panelVal.sellingUOM !== 'PKT') {
                enteredVal = (panelVal.crates * panelVal.ppc) + panelVal.qty;
                if ((availToOrder - enteredVal) < 0) {
                  this.toastr.error('Enough stock not available', 'Call Desk', {
                    timeOut: 1000
                  });
...
// new values
    ...
    this.callDeskService.updateSalesOrder(updateArray).subscribe((response) => {
      ...
      //this.itemDetails = '';
      for (var i = 0; i < updateArray.salesOrderLine.length; i++) {
        // let PacketAndCrt;
