...
// import { CreateIndentService } from 'src/app/shared/service/create-indent.service';
...
@Component({
  selector: 'app-indent-for-cust-by-emp',
  templateUrl: './indent-for-cust-by-emp.component.html',
  styleUrls: ['./indent-for-cust-by-emp.component.css']
})
export class IndentForCustByEmpComponent implements OnInit {
    totalTax: any[] = [];
    custBilling: any;
    occupations: any;
    custNm: any;
  ...
  custId: any;
      ...
      this.data = JSON.parse(localStorage.getItem('data'));
        ...
        this.activatedRoute.params.subscribe(params => {
          if (params['customerId']) {
            this.accountNumber= params['customerId'];
          }
        }); 
    ...
    submit(form: NgForm) {
      ...
      form.value.newIndtLItem.forEach(e => {
        this.itemDetails += e.itemDesc.itemCode + ":" + e.packets + 'P,' ? e.packets + 'P,' : e.crt + 'C,';
      ...
      this.CreateIndentService.createSalesOrderAndItems(this.SalesOrderHeader.value).subscribe(res => {
        ...
        let smsTemplate = 'Dear :' + this.customerShippingDetails.accountNumber + '' + ', Indent created for ' +
          this.SalesOrderHeader.value.bookingDate + ' Shift: ' + this.SalesOrderHeader.value.shiftId.shiftCode + '' + '  is ' + '\n' + this.itemDetails + '\n' + '   Worth of Rs.' + this.sumTotal;
