...
@Component({
  selector: 'app-create-master-indent-manager',
  templateUrl: './create-master-indent-manager.component.html',
  styleUrls: ['./create-master-indent-manager.component.css']
})
export class CreateMasterIndentManagerComponent implements OnInit {
  submitted = false;
  itemCodeDesc: any = [];
  ...
  constructor(
    ...
    debugger
    this.accountNumber=JSON.parse(localStorage.getItem('create-indent-manager'));
  }
  ...
  getCustBillingDetailsByAcctNo() {
    this.masterIndentService.getCustBillingDetailsByAcctNo(this.accountNumber).subscribe(res => {
  ...
  getCustShippingDetailsByAcctNo() {
    this.masterIndentService.getCustShippingDetailsByAcctNo(this.accountNumber).subscribe(res => {
      this.MasterIndentHeader.patchValue({
  ...
  getCustomerBalanceByAcctNo() {
    this.masterIndentService.getCustomerBalanceByAcctNo(this.accountNumber).subscribe(
      res => {
        this.balanceNo = res;
      });
  }
  ...
  submit(form: NgForm) {
    ...
    this.masterIndentService.createMstrIndent(this.MasterIndentHeader.value).subscribe(res => {
      ...
      this.router.navigateByUrl('mkt-dairy-manager/master-indent/list-master-indents-manager');
    }, error => {
