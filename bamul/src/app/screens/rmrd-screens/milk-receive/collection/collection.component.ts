...
export class CollectionComponent implements OnInit , OnDestroy {
  ...
  addMoreDisabled: boolean=true;
  // tslint:disable-next-line: no-shadowed-variable
  // tslint:disable-next-line: max-line-length
  constructor(private fb: FormBuilder, private MilkCollectionService: MilkCollectionService, private SharedService: SharedService, private toastr: ToastrService, private router: Router , private route: ActivatedRoute) {
  }
  ngOnInit() {
  ...
  routeonChange(event) {
    ...
    this.onChange(this.routeNosocietylogs[0].tripPoints[0].routePointCode)
    ...
    //     }
        ...
        this.MilkCollectionService.saveMilkCollection(this.postData).subscribe(
          res => {
            ...
            //this.router.navigateByUrl(`/rmrd-operator/milk-receive/edit-collection/${res['shipmentHeaderId']}`);
            this.router.navigate([`../edit-collection` , res['shipmentHeaderId']] , { relativeTo: this.route });
          }, error => {
              if(error.status == 409) {
                this.toastr.error(error.error.message,'Error' , {
                  timeOut: 5000
