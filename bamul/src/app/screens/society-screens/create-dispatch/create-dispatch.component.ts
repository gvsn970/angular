...
export class CreateDispatchComponent implements OnInit {
  ...
  dispatchVal(val) {
    ...
    if (val === 'Route') {
      ...
      this.milkSocietyService.getTripDetailsByScheduleDateAndShiftAndRoutePointCode(this.SharedService.getDate(), this.SharedService.getShift(), vendorNum).subscribe((response) => {
...
save() {
  ...
  if (Object.keys(this.societyDispatchDetails).length > 0) {
    objVal.vehicleNumber = this.societyDispatchDetails.vehicleNumber;
    objVal.driverName = this.societyDispatchDetails.driverName;
  ...
  objVal.shift = this.SharedService.getShift();
  objVal.tripDate = this.SharedService.getDate();
  ...
  if (this.ngForm.valid) {
    this.milkSocietyService.saveSociety(objVal).subscribe((response) => {
    });
